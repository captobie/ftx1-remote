    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                HStack(spacing: 12) {
                    VFODisplayBox(label: "VFO A", frequencyHz: viewModel.rigState.frequencyHz, isActive: true, mode: viewModel.rigState.mode.displayName, callsign: viewModel.rigState.mode == .c4fm ? viewModel.rigState.c4fmCallsign : nil, reflector: viewModel.rigState.mode == .c4fm ? viewModel.rigState.c4fmReflector : nil, onSetFrequency: { viewModel.send(.setFrequency(hz: $0)) })
                    vfoSwapButton
