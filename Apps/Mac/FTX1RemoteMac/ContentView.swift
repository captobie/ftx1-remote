...
struct ContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack(spacing: 12) {
                VFODisplayBox(label: "VFO A", frequencyHz: hub.rigState.frequencyHz, isActive: true, mode: hub.rigState.mode.displayName, callsign: hub.rigState.mode == .c4fm ? hub.rigState.c4fmCallsign : nil, reflector: hub.rigState.mode == .c4fm ? hub.rigState.c4fmReflector : nil, onSetFrequency: { hub.send(.setFrequency(hz: $0)) })
                vfoSwapButton
